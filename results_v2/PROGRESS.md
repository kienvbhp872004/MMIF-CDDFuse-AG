...
| | |
|---|---|
| Latest update | 2026-05-12 14:30 |
| Variants completed | 4/4 Module A + 4/4 Module B + Combined (light) + **CDDFuse-Paper-MIF (full 120 ep / 2-phase P100 GPU)** |
| Latest activity | **CDDFuse-Paper-MIF vs CDDFuse_MIF.pth (gốc paper)**: MIXED 3 SIG pooled, **QM δ chỉ -0.137 trivial** (vs Combined light -0.587 LARGE!). QSF +0.350 m SIG, CT 7 + PET 10 SIG per-modal. |
| Status | ⚠️ **PHÁT HIỆN QUAN TRỌNG**: Trade-off NABF↑/QM↓ mà 9 light retrain chỉ ra **CÓ THỂ LÀ ARTIFACT của under-train**, không phải inherent. Paper-faithful train (encoder/decoder co-adapt) gần như KHÔNG có QM regression. |
| Next planned | Train Combined-Gated-Saliency với paper procedure (120 ep / 2-phase) để confirm/refute hypothesis |

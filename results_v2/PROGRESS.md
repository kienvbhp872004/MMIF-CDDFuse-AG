...
| | |
|---|---|
| Latest update | 2026-05-12 21:57 |
| Variants completed | Light: 4/4 A + 4/4 B + Combined-Light + **Paper: CDDFuse-Paper-MIF + Combined-Paper-MIF ✓** |
| Latest activity | **Combined-Paper-MIF (paper-faithful 120 ep / 2-phase P100 GPU)**: MIXED 2 SIG. **Trade-off NABF↑/QM↓ BIẾN MẤT** (Cliff's δ small thay vì LARGE như light). Combined ≈ baseline khi train đầy đủ. |
| Status | ✅ **HYPOTHESIS CONFIRMED**: Light retrain trade-off là **artifact** của under-train + freeze. Paper-faithful training neutralize cải tiến Module A+B. Per-modal: CT 10 SIG, PET 10 SIG nhưng pooled chỉ 2. |
| Next planned | Thesis writeup: framing "light retrain rapid screening identified candidates → paper-faithful confirmed gains too small to claim improvement → contribution: methodology insights" |
